...
def getAffinity_Matrix(src,affinity):
    # [width, height]
    # print(affinity.shape)
    src1 = src
    # 计算向量的模
    src_ = torch.sqrt((src1[:,:, :] ** 2).sum(dim=-1))  # 8, 3
    src2 = src
    for batch_id in range(affinity.shape[0]):
        for idx in range(affinity.shape[1]):
            affinity[batch_id, idx, :] = torch.mul(src1[batch_id, idx, :], src2[batch_id, :, :]).sum(dim=-1)
            affinity[batch_id, idx, :] = affinity[batch_id, idx, :] / src_[batch_id, idx]
        for idx in range(affinity.shape[1]):
            # continue
            affinity[batch_id, :, idx] = affinity[batch_id, :, idx] / src_[batch_id, idx]
    return affinity
...
def display(affinity):
    plt.imshow(affinity.detach().cpu())
    plt.colorbar()
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.proj=nn.Linear(self.joints_num*3, d_model) # 54: 18jointsx3
        self.proj2=nn.Linear(self.joints_num*3, d_model)
        self.proj_inverse=nn.Linear(d_model,self.joints_num*3)
        self.l1=nn.Linear(d_model, d_model*4)
        self.l2=nn.Linear(d_model*4, d_model*self.output_time)
    ...
    def forward(self, src_seq, trg_seq, input_seq, use=None):
        ...
        others=input_seq[:,:,:,:].reshape(input_seq.shape[0],-1,self.joints_num*3)
        ...
        dec_output=dec_output.view(dec_output.shape[0], self.output_time , self.d_model)
