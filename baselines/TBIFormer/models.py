...
class Tem_ID_Encoder(nn.Module):
    def __init__(self, d_model, dropout=0.1,
        ...
        ae = self.build_agent_enc(max_a_len)
    ...
    def get_agent_enc(self, num_a, num_p, num_t, a_offset, agent_enc_shuffle):
        if agent_enc_shuffle is None:
            ae = self.ae[:, a_offset: num_a + a_offset]
        else:
            ae = self.ae[:, agent_enc_shuffle]
        ae = ae.repeat_interleave(num_p*num_t, dim=1)
        return ae

    def forward(self, x, num_a, num_p, num_t, t_offset=0, a_offset=0):
        index = list(np.arange(0, num_a))
        agent_enc_shuffle = random.sample(index, num_a)
        pos_enc = self.get_pos_enc(num_a, num_p, num_t, t_offset)
        ae_enc = self.get_agent_enc(num_a, num_p, num_t, a_offset, agent_enc_shuffle)
        x = x + pos_enc + ae_enc     #  Temporal Encoding + Identity Encoding
...
class TBIFormerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.embeddings_table = nn.Embedding(10, d_k * n_head)  #  for  shared TRPE embedding
...
class TBIFormer(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(nn.Conv1d(in_channels=opt.joints_num*3, out_channels=d_model, kernel_size=kernel_size1,
                                             bias=False),
                                   nn.ReLU(inplace=False),
        ...
        self.proj_inverse=nn.Linear(d_model, opt.joints_num*3)
        self.l1=nn.Linear(d_model, d_model*8)
        self.l2=nn.Linear(d_model*8, d_model*opt.output_time)
    ...
    def forward(self, src, n_person, trj_dist):
        ...
        # index = [[0,1,10,11,12,13],[2, 3, 4, 5], [6, 7, 8, 9], [14, 15, 16], [17, 18, 19]]  # 5 body parts
        index = [[0,1,8,9,10,11], [2, 3, 4], [5, 6, 7], [12, 13, 14], [15, 16, 17]]
